...
describe("UsuarioService", () => {
  ...
  describe('UsuarioService - listar', () => {
    ...
    it('Munícipe deve receber erro ao acessar outro usuário', async () => {
      ...
      const req = { user_id: '6871d7b722e1cf0b2d0f3aab', params: { id: 'outro-id' } };
    ...
    it('Secretário deve listar apenas usuários da mesma secretaria', async () => {
      ...
      repositoryMock.buscarPorID.mockResolvedValueOnce(usuarioDaMesmaSecretaria); // usuário pesquisado
  ...
  describe('UsuarioService - criar', () => {
    it('Deve lançar erro se o usuário logado for munícipe', async () => {
      ...
      await expect(service.criar({ email: 'teste@email.com' }, req))
        .rejects
        .toThrow(CustomError);
    ...
    it('Deve validar o email e criar usuário com senha hasheada', async () => {
      ...
      expect(repositoryMock.criar).toHaveBeenCalledWith({ ...dados, senha: 'senha-hash' });
  ...
  describe('UsuarioService - criarComSenha', () => {
    it('Deve criar munícipe com senha hasheada e grupo correto', async () => {
      ...
      expect(repositoryMock.buscarPorEmail).toHaveBeenCalledWith('joao@teste.com', null);
      expect(AuthHelper.hashPassword).toHaveBeenCalledWith('Senha@123');
      expect(grupoRepositoryMock.buscarPorNome).toHaveBeenCalledWith('municipe');
    ...
    it('Deve lançar erro se email já estiver cadastrado', async () => {
      repositoryMock.buscarPorEmail.mockResolvedValue({ email: 'existe@teste.com' });

      await expect(service.criarComSenha({
        nome: 'Repetido',
  ...
  describe('UsuarioService - atualizar', () => {
    ...
    it('Admin deve atualizar qualquer usuário com sucesso', async () => {
      ...
      const id = 'outro-id';
      const parsedData = { nome: 'Atualizado', email: 'x@x.com', senha: '123', nivel_acesso: {}, grupo: 'x' };
      ...
      expect(repositoryMock.atualizar).toHaveBeenCalledWith(id, expect.objectContaining({ nome: 'Atualizado' }));
      expect(resultado).toEqual(esperado);
    });
    ...
    it('Usuário comum pode atualizar a si mesmo (sem campos sensíveis)', async () => {
      ...
      repositoryMock.atualizar.mockResolvedValue({ _id: id, nome: 'Atualizado' });
    ...
    it('Deve lançar erro se usuário a ser atualizado não existir', async () => {
      ...
      await expect(service.atualizar(id, {}, req)).rejects.toThrow('Usuário não encontrado.');
    });
  });
  ...
  describe('UsuarioService - atualizarFoto', () => {
    ...
    it('Admin não pode atualizar a foto de outro usuário', async () => {
      ...
      await expect(service.atualizarFoto(id, parsedData, req)).rejects.toThrow(CustomError);
    ...
    it('Deve lançar erro se usuário não existir', async () => {
      ...
      await expect(service.atualizarFoto(id, parsedData, req)).rejects.toThrow('Usuário não encontrado.');
    });
  });
  ...
  describe('UsuarioService - ensureUserExists', () => {
    ...
    it('Deve lançar erro quando usuário não existir', async () => {
      ...
      await expect(service.ensureUserExists('nao-existe-id')).rejects.toThrow(CustomError);
    });
  });
})
