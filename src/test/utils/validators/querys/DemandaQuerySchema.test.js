import { DemandaQuerySchema, enderecoSchema } from '../../../../utils/validators/schemas/zod/querys/DemandaQuerySchema.js';
...
describe('DemandaQuerySchema', () => {
  ...
  describe('Validação de tipo (enum)', () => {
    ...
    it('deve rejeitar tipo inválido', () => {
      ...
      if(!result.success) expect(result.error.issues[0].message).toMatch(/Invalid enum value/);
    });
  });
  ...
  describe('Validação de usuários (ObjectId)', () => {
    ...
    it('deve rejeitar usuário com ObjectId inválido', () => {
      ...
      expect(result.success).toBe(false);
      if(!result.success) expect(result.error.issues[0].message).toBe("Usuário inválido. Tente novamente!");
    });
  });
  ...
  describe('Validação de page e limite', () => {
    it('deve rejeitar page menor que 1', () => {
      ...
      expect(result.success).toBe(false);
      if(!result.success) expect(result.error.issues[0].message).toBe("Page deve ser um número inteiro maior que 0");
    });

    it('deve rejeitar limite menor que 1', () => {
      const result = DemandaQuerySchema.safeParse({ limite: "0" });
      expect(result.success).toBe(false);
      if(!result.success) expect(result.error.issues[0].message).toBe("Limite deve ser um número inteiro entre 1 e 100");
    });
