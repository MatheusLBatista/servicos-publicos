...
describe('Rotas de tipoDemanda', () => {
  it('GET - Deve retornar uma lista dos tiposDemandas cadastradas', async () => {
    const res = await request(app).get("/tipoDemanda");
    //console.log(res.body);
    expect(res.status).toBe(200);
    expect(res.body.message).toBe("Requisição bem-sucedida");
  });
  it('GET - Deve retornar um tipoDemanda pelo ID', async () => {
    const res = await request(app).get("/tipoDemanda/6832ad0c109564baed4cda14");
    //console.log(res.body);
    expect(res.status).toBe(200);
    expect(res.body.message).toBe("Requisição bem-sucedida");
  ...
  it('GET - Deve retornar erro de recurso não encontrado em tipoDemanda pelo ID não existente', async () => {
    const res = await request(app).get("/tipoDemanda/6832ad0c109564baed4cda11");
    //console.log(res.body);
   ...
   it('POST - Deve cadastrar um novo tipoDemanda com sucesso', async () => {
    ...
    const res = await request(app).post("/tipoDemanda").send(novoTipoDemanda);
    //console.log(res.body);
  ...
  it('PATCH - Deve atualizar parcialmente um tipoDemanda', async () => {
    ...
    const res = await request(app).patch(`/tipoDemanda/6832ad0c109564baed4cda14`).send(atualizacao);
    expect(res.status).toBe(200);
...
/*
  it('DELETE - Deve deletar um tipoDemanda com sucesso', async () => {
    const res = await request(app).delete("/tipoDemanda/68353b31ed620b3827f3f06c");
    //console.log(res.body);
    expect(res.status).toBe(200);
    expect(res.body.message).toBe("TipoDemanda excluída com sucesso.");
    expect(res.body.data._id).toBe("68353b31ed620b3827f3f06c");
  });
*/
